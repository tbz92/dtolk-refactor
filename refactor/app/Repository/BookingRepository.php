...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id): array
    {
        ...
        if ($cUser?->is('translator')) {
            ...
            $jobs = $jobs->pluck('jobs')->sortBy('due')->all();
        }

        // This IF block can be removed and $jobs can be initialized as an empty array. But it's more readable.
        if (!empty($jobs)) {
    ...
    public function getUsersJobsHistory($userId, Request $request): array
    {
        ...
        if ($cUser?->is('translator')) {
            $usertype = 'translator';
            $jobs = $normalJobs = Job::getTranslatorJobsHistoric($cUser->id, 'historic', $pageNum);
    ...
    public function store($user, $data)
    {
        ...
        try {
            ...
            DB::commit();
            return self::processDataAndCreate($data, $consumer_type, $user);
        } catch (\Exception $e) {
            DB::rollback();
        }
        /*
         *
         * ******* NO NEED FOR THIS SEGMENT AS IT IS NOT BEING USED ANYWHERE *****
         *
        ...
        ******Error will be thrown from StoreBookingRequest because condition added in authorize method.*****
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if (
                isset($requestdata['customer_email']) && count(
    ...
    public function flagJobColumnToIgnore($id, $column)
    {
        /*
         * $column possible values (ignore, ignore_expired)
         *
         * Also, can be moved to model. If multiple occurence is present like this for different model
         * We can create a BaseModel and extends every model with it.
         * example:
         * class BaseModel extends Model {}
         * class Job extends BaseModel {}
         * */
        Job::find('id', $id)->update([$column => 1]);
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    /* MY functions*/
    private static function processDataAndCreate(&$data, $consumer_type, $cuser): array
    {
        $immediatetime = 5;
        $data['customer_phone_type'] = !empty($data['customer_phone_type']) ? 'yes' : 'no';
        $data['gender'] = in_array('male', $data['job_for']) ? 'male' : 'female';
        $data['b_created_at'] = date('Y-m-d H:i:s');
        $data['by_admin'] = !empty($data['by_admin']) ? $data['by_admin'] : 'no';
